...
public class UserInfoAdapter {
	...
	public final String creatJiaoTable = "create table "
	        + DATABASE_JIAO_TABLE_NAME + " (" + KEY_ID
	        + " integer primary key autoincrement, " + KEY_USER_ID
	        + " text not null," + KEY_USER_PASSWORD + " text not null);";

	public final String creatTuTable = "create table " + DATABASE_TU_TABLE_NAME
	        + " (" + KEY_ID + " integer primary key autoincrement, "
	        + KEY_USER_ID + " text not null," + KEY_USER_PASSWORD
	        + " text not null);";
	...
	public class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
			...
			Cursor c = null;
			try {
				c = db.rawQuery("select * from " + DATABASE_JIAO_TABLE_NAME,
				        null);
				c.moveToFirst();
				while (!c.isAfterLast()) {
	...
	public Cursor fetchItemByUserId(String table, String userId)
	        throws SQLException {
		return mDb.rawQuery("SELECT * FROM " + table + " WHERE " + KEY_USER_ID
		        + " = '" + userId + "'", null);
	}
