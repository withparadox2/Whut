...
public class WhutGlobal {
	 ...
	 * WhichAction = 1时，执行登陆。 WhichAction = 2时，执行查询课表。 WhichAction =
	 * 3时，执行查询个人课表。 WhichAction = 4时，执行查询成绩。 WhichAction = 5时，执行评教。 WhichAction
	 * = 6时，执行保存评教 WhichAction = 7时，执行提交评教 WhichAction = 8时，执行请求个人课表Post
	 * WhichAction = 9时，执行登陆图书馆 WhichAction = 10时，执行借阅查询 WhichAction =
	 * 10时，执行进入续借
	 */
	...
	/**
	 * BOOKLIST contains a one-dimensional-array, of which the length is 2 one
	 * for book-detail, another for bookNumber
	 */
