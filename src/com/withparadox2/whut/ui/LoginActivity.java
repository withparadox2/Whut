...
public class LoginActivity extends Activity {
		...
		public void onClick(View v) {
			...
			if (httpOperation.isConnect(LoginActivity.this)) {
				...
				if (!userIdText.equals("") && !userPasswordText.equals("")) {
					...
					if (jiaoTuFlag) {
						Toast.makeText(LoginActivity.this, "密码或学号不能为空...",
						        Toast.LENGTH_LONG).show();
					} else {
						Toast.makeText(LoginActivity.this, "密码或卡号不能为空...",
						        Toast.LENGTH_LONG).show();
					}
				}
			} else {
				Toast.makeText(LoginActivity.this, "貌似没有联网...",
				        Toast.LENGTH_LONG).show();
			}
		}
	}
		...
		public void onClick(View v) {
			...
			if (popupWindow == null) {
				...
				popupWindow = new PopupWindow(popupListView,
				        userIdEdit.getWidth(), LayoutParams.WRAP_CONTENT);
	...
	private void getListFromDb(String table) {
		...
		if (c.getCount() != 0) {
			c.moveToFirst();
			userIdList.add(c.getString(c
			        .getColumnIndexOrThrow(UserInfoAdapter.KEY_USER_ID)));
			userPasswordList.add(c.getString(c
			        .getColumnIndexOrThrow(UserInfoAdapter.KEY_USER_PASSWORD)));
			while (!c.isLast()) {
				c.moveToNext();
				userIdList.add(c.getString(c
				        .getColumnIndexOrThrow(UserInfoAdapter.KEY_USER_ID)));
		...
		public View getView(int position, View converView, ViewGroup parent) {
			...
			holder.popupText.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					userPasswordEdit.setSelection(userPasswordEdit.getText()
					        .length());
				}
			});
			...
			holder.deleteButton.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View arg0) {
					// TODO Auto-generated method stub
					if (jiaoTuFlag) {
						long id = dbHelper.returnIdByUserId(
						        UserInfoAdapter.DATABASE_JIAO_TABLE_NAME,
						        userIdText);
						if (id != 0) {
							dbHelper.deleteItem(
							        UserInfoAdapter.DATABASE_JIAO_TABLE_NAME,
						...
						long id = dbHelper.returnIdByUserId(
						        UserInfoAdapter.DATABASE_TU_TABLE_NAME,
						        userIdText);
						if (id != 0) {
							dbHelper.deleteItem(
							        UserInfoAdapter.DATABASE_TU_TABLE_NAME, id);
						}
						getListFromDb(UserInfoAdapter.DATABASE_TU_TABLE_NAME);
					}
					popupListAdapter.notifyDataSetChanged();
	...
	public class UpdateUIHandler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case 6:
				...
				Toast.makeText(LoginActivity.this, "不好意思，数据出现问题...",
				        Toast.LENGTH_LONG).show();
				break;
			case 7:
				progressDialog.dismiss();
				Toast.makeText(LoginActivity.this, "不好意思，网络出现问题...",
				        Toast.LENGTH_LONG).show();
				break;
			case 5:
				progressDialog.dismiss();
				if (WhutGlobal.JUMP_OR_NOT && (!cancelDialogByHand)) {
					...
					if (!cancelDialogByHand) {
						if (jiaoTuFlag) {
							Toast.makeText(LoginActivity.this, "密码或学号错误...",
							        Toast.LENGTH_LONG).show();
						} else {
							Toast.makeText(LoginActivity.this, "密码或卡号错误...",
							        Toast.LENGTH_LONG).show();
						}
					}
				}
